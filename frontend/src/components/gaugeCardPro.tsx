...
export default function GaugeCardPro({ label, value, max, unit, color, threshold }: Props) {
  ...
  return (
    <div className="glass rounded-2xl p-5 shadow-lg hover:shadow-neon-lg transition-all duration-300 border border-white/10">
      <div className="flex items-start justify-between mb-3">
        <div>
          <p className="text-xs font-medium uppercase tracking-wider text-slate-400">{label}</p>
          <p className="mt-1 text-2xl font-bold text-white">{value.toFixed(1)}</p>
          <p className="text-xs text-slate-400">{unit}</p>
        </div>
        {exceeded && <AlertCircle className="w-4 h-4 text-red-400 animate-pulse" />}
      </div>
