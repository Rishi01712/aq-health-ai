...
export default function GaugeCardPro({ label, value, max, unit, color, threshold }: Props) {
  ...
  return (
    <div className="glass rounded-2xl p-4 sm:p-5 shadow-lg hover:shadow-neon-lg transition-all duration-300 border border-white/10">
      <div className="flex items-start justify-between mb-2 sm:mb-3">
        <div>
          <p className="text-xs sm:text-sm font-medium uppercase tracking-wider text-slate-400">{label}</p>
          <p className="mt-1 text-xl sm:text-2xl font-bold text-white">{value.toFixed(1)}</p>
          <p className="text-xs text-slate-400">{unit}</p>
        </div>
        {exceeded && <AlertCircle className="w-4 h-4 sm:w-5 h-5 text-red-400 animate-pulse flex-shrink-0" />}
      </div>

      <div className="h-2 w-full bg-slate-700 rounded-full overflow-hidden mb-2 sm:mb-3">
        <div
